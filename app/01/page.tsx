...
function TerminalFlipScore({ num, showBust }: { num: number, showBust: boolean }) {
  ...
  return (
    <div className="flex flex-row justify-center items-end" style={{ gap: '0.10em' }}>
  ...
  return (
    <div onClick={onChange} className="w-10 h-10 flex items-center justify-center cursor-pointer" tabIndex={0}>
      <span className="inline-block w-8 h-8 rounded-full border-2 border-yellow-400 bg-transparent relative">
        {enabled && <span className="absolute left-1 top-1 w-6 h-6 rounded-full bg-yellow-400" />}
      </span>
    </div>
  );
}

// MO/OO custom switch（橫長形滑動區塊，滑塊為橫長白色長方形，三條直線防滑）
function SwitchBox({ checked, onChange }: { checked: boolean, onChange: () => void }) {
  return (
    <div
      ...
      tabIndex={0}
      onClick={onChange}
      className="relative flex items-center w-24 h-11 bg-zinc-700 rounded-lg cursor-pointer select-none transition"
      style={{ borderRadius: 8 }}
      aria-label="MO/OO 切換">
      {/* 軌道底色效果 */}
      <div className={`absolute inset-0 rounded-lg transition`} style={{ backgroundColor: checked ? "rgba(59,130,246,0.18)" : "rgba(180,192,212,0.06)" }} />
      {/* 滑塊區：橫長方型移動於軌道內 */}
      <div className={`absolute top-1 left-1 transition-transform duration-200`}
        style={{ transform: checked ? "translateX(3.7rem)" : "translateX(0)" }}>
        <div className="switch-rect w-12 h-8 bg-white rounded flex items-center shadow ring-2 ring-zinc-400 px-2">
          {/* 三條防滑直線 */}
          <div className="flex flex-col gap-1 mx-auto">
            <div className="h-6 w-[2px] bg-zinc-700 rounded"/>
            <div className="h-6 w-[2px] bg-zinc-700 rounded"/>
            <div className="h-6 w-[2px] bg-zinc-700 rounded"/>
          </div>
        </div>
...
export default function Page01() {
  ...
  const btnClass = "w-20 h-20 flex items-center justify-center shadow-lg p-0";
  ...
  return (
    ...
    >
      <main className="flex flex-col w-full h-[100svh] max-w-full flex-1 relative">
        ...
        {/* 右下回合切換 */}
        <div className="absolute bottom-[7.5rem] right-5 z-50">
        ...
        <div className="flex-1 flex flex-row items-stretch w-full h-full">
          ...
          <div className="flex flex-col justify-start items-center flex-[1_1_0%] min-w-[220px] max-w-[340px] px-3">
            <div ref={historyBox}
                className="rounded-2xl shadow-inner flex flex-col w-full max-h-[calc(100vh-128px)] overflow-y-scroll custom-scrollbar py-8 transition-all"
            >
          ...
          {/* 右側本回合分數，接近頂部、間距更窄 */}
          <div className="flex flex-col justify-start items-center flex-[1_1_0%] min-w-[220px] max-w-[340px] px-3 pt-8 pb-16">
            <div className="flex flex-col items-center w-full gap-y-3 mt-8 mb-9">
              {[0, 1, 2].map(i => {
        ...
        <div className="relative">
          <div className="absolute left-0 right-0 top-0 h-5 bg-zinc-800" />
          <div className="flex items-center justify-center gap-10 w-full py-8 bg-gradient-to-t from-black to-zinc-900 z-10 relative border-t border-zinc-700">
