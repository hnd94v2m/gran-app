...
function TerminalFlipDigit({ digit }: { digit: string }) {
  ...
  return (
    <span
      className="terminal-digit select-none bg-black text-green-400 font-mono font-extrabold px-8 py-10 rounded shadow-2xl text-[20rem] leading-none drop-shadow-xl flex items-end"
  ...
  if (showBust) {
    return (
      <div className="flex flex-row items-end justify-center">
        <span
          className="terminal-digit font-mono text-red-500 bg-black font-black text-[20rem] px-8 py-10 rounded shadow-2xl flex items-center"
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ backgroundColor: checked ? "rgba(59, 130, 246, 0.3)" : "rgba(212, 212, 212, 0.2)" }}
      />
      {/* 可滑動的橫長方形滑塊 */}
      <div
      ...
      >
        <div className="switch-rectangle w-12 h-7 bg-white rounded-md flex flex-col justify-center shadow ring-2 ring-zinc-400">
          {/* 止滑條 (三條垂直線) */}
          <div className="flex justify-between px-2">
            <span className="w-[2px] h-5 bg-zinc-700 rounded" />
            <span className="w-[2px] h-5 bg-zinc-700 rounded" />
            <span className="w-[2px] h-5 bg-zinc-700 rounded" />
          </div>
        </div>
      </div>
      {/* 左右icon為 MO (左) 和 OO(右) 的直覺位置, 文字去掉 */}
      {/* 你可自行加圖標，這裡暫留空 */}
    </div>
  );
}
...
function FatBullSwitch({ enabled, onChange }: { enabled: boolean; onChange: () => void }) {
  return (
    <div
      ...
      tabIndex={0}
      aria-label={`Fat Bull 功能 ${enabled ? "開啟" : "關閉"}`}
    >
      {enabled ? (
...
export default function Page01() {
  ...
  const [fatBullEnabled, setFatBullEnabled] = useState(true); // 預設開
  const [moMode, setMoMode] = useState(true); // true=MO，false=OO
  ...
  const btnClass =
    "w-20 h-20 flex items-center justify-center shadow-lg p-0";
  ...
  return (
    ...
    >
      <main className="flex flex-col w-full h-[100svh] max-w-full flex-1 relative">
        ...
        {/* 右下角回合切換按鈕（比玩家條高度略高） */}
        <div className="absolute bottom-[7.5rem] right-6 z-50">
          <button
        ...
        <div className="flex-1 flex flex-row items-stretch w-full h-full">
                  ...
                  {history.map((sum, idx) => (
                    <React.Fragment key={idx}>
                      <div
                        ...
                        className="text-4xl font-bold text-gray-900 py-5 border border-black select-none"
                      >
                        R{idx + 1}
                      </div>
                      <div
                        style={{ backgroundColor: bgColorByIndex(idx, history.length) }}
                        className="text-6xl font-extrabold py-5 border border-black select-none"
                      >
                        {sum}
                      </div>
                    </React.Fragment>
          ...
          {/* 中間最大分數區，間距極窄 */}
          <div
            className="flex flex-col items-center justify-center flex-[2_2_0%] max-w-[66vw] min-w-0 min-h-[520px]"
          >
            <TerminalFlipScore num={score >= 0 ? score : 0} showBust={showBust} />
          </div>

          {/* 右側本回合分數，垂直對齊、下移，間距縮小 */}
          <div
            className="flex flex-col justify-end items-center flex-[1_1_0%] min-w-[220px] max-w-[340px] px-3 pb-16 pt-8"
          >
            <div className="flex flex-col items-center w-full gap-y-3 mb-8 mt-16">
              {[0, 1, 2].map((i) => {
                const highlight =
        ...
        {/* 下方玩家條，加底色分隔線 */}
        <div className="flex items-center justify-center gap-10 w-full py-8 bg-gradient-to-t from-black to-zinc-900 border-t border-zinc-700">
