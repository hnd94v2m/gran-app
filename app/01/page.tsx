...
const RED_BUTTON_SEGMENT_ID: number = 84; // 依你實測知的紅按鈕ID
  ...
  return (
    <div onClick={onChange} className="w-10 h-10 flex items-center justify-center cursor-pointer" tabIndex={0}>
      <span className="inline-block w-8 h-8 rounded-full border-2 border-yellow-400 bg-transparent relative flex items-center justify-center">
        {enabled && (
          <span
            style={{ width: 18, height: 18, borderRadius: "999px", background: "#fde047", display: "inline-block" }}
          />
        )}
      </span>
    </div>
...
export default function Page01() {
  ...
  useEffect(() => {
    ...
    granboard.segmentHitCallback = (segment: Segment) => {
      if (hitLock.current) {
        console.log("hitLock 鎖定中，忽略鏢", segment);
        return;
      }
      if (roundEnded.current) {
        console.log("回合結束中，忽略鏢", segment);
        return;
      }
      ...
      setCurrThrows(prev => {
        ...
        if (nowThrows.length >= 3) {
          console.warn("本回合鏢數已達3支，忽略本鏢");
          return nowThrows;
        }
        const newThrows = [...nowThrows, hitVal];
        if (nowThrows.length === 0) {
          setLastValidScore(scoreRef.current);
          console.log("設定 lastValidScore:", scoreRef.current);
        }
        ...
        if (isBust) {
          console.log("Bust 判定，回合分數無效，還原至", lastValidScore);
          setBust(true);
        ...
        if (newThrows.length === 3) {
          console.log("三鏢完畢，自動結束回合並計分", newThrows);
  ...
  const endRound = () => {
    ...
    console.log("手動結束回合", currThrowsRef.current);
    endRoundWithThrows(currThrowsRef.current);
  };
        ...
        <div className="absolute top-4 right-4 z-50">
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-72 bg-zinc-900 border border-zinc-700 rounded shadow-lg flex flex-col select-none z-[999]">
              ...
              <button className="px-8 py-4 text-2xl text-left hover:bg-zinc-700 focus:bg-zinc-700" onClick={resetGame}>
                重新開始
              </button>
              <button className="px-8 py-4 text-2xl text-left hover:bg-zinc-700 focus:bg-zinc-700" onClick={retryCurrentRound}>
                重投
              </button>
              ...
              <button className="px-8 py-4 text-2xl text-left border-t border-zinc-700 hover:bg-zinc-700" onClick={goHome}>
                返回首頁
              </button>
            </div>
          )}
        ...
        <div className="flex flex-row w-full flex-1 items-start" style={{ paddingTop: `${MENU_BTN_HEIGHT}px` }}>
          ...
          <div className="flex flex-col flex-[2_2_0%] max-w-[66vw] w-full items-center justify-between h-full pt-0">
            <div className="flex flex-1 items-center justify-center w-full h-full" style={{ maxWidth: "66vw", minHeight: "12rem" }}>
              <div style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", minWidth: "min(100vw,1200px)" }}>
            ...
            <div className="flex items-center justify-center gap-10 w-full py-8 bg-gradient-to-t from-black via-zinc-950/80">
              <span className="inline-block w-24 h-24 rounded-full bg-zinc-700 text-[4.5rem] flex items-center justify-center select-none">
                {avatar}
              </span>
              ...
              {[0, 1, 2].map(i => {
                ...
                return (
                  <div key={i} className="flex flex-col items-end w-full">
                    <div
                      style={{ width: threeMarkBoxWidth, height: threeMarkBoxHeight, borderRadius: 0 }}
